#' Dotplots of district-level risk group proportion estimates
run_commit_push("plot_within-between-country-variation") #' [x]

#' Temporal variation in risk group proportion estimates
run_commit_push("plot_temporal-variation") #' [x]

#' Kernel density estimator plots of risk group proportion density by age
run_commit_push("plot_age-variation") #' [x]

#' Choropleth maps for each age group and risk group across all 13 countries
run_commit_push("plot_continental-map") #' [x]

#' Dotplots and cloropoth of cohabiting and non-regular partner(s) risk groups (women aged 20-29)
run_commit_push("plot_aids-abstract") #' [x]

#' Choropleth contrasting direct estimates with modeled estimates, and illustrating benefits of the later
run_commit_push("plot_model-direct-benefits") #' [ ]

#' Barplot of the information criteria (DIC, WAIC, CPO) for each model
run_commit_push("process_information-criteria") #' [/]

#' Variance based sensitivity analysis
run_commit_push("process_variance-proportions") #' [/]

#' Posterior predictive sampling to generate coverage estimates
run_commit_push("process_coverage") #' [ ]

#' Calculate and plot HIV prevalence by risk group
run_commit_push("process_prevalence") #' [x]

#' Calculate and plot HIV incidence by risk group
run_commit_push("process_incidence") #' [ ]

#' Analysis and plots of number of infections reached by different prioritisation strategies
run_commit_push("plot_infections-reached") #' [ ]
