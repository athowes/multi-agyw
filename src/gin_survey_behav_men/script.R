...
# orderly::orderly_develop_start("gin_survey_behav_men")
# setwd("src/gin_survey_behav_men")

#' ISO3 country code
iso3 <- "GIN"

areas <- read_sf("depends/gin_areas.geojson") %>% st_make_valid()
areas_wide <- naomi::spread_areas(areas)

surveys <- create_surveys_dhs(iso3, survey_characteristics = 24) %>%
  filter(as.numeric(SurveyYear) > 1999)
...
write_csv(survey_indicators, "gin_survey_indicators_sexbehav.csv", na = "")
...
write_csv(hiv_indicators, "gin_hiv_indicators_sexbehav.csv", na = "")
