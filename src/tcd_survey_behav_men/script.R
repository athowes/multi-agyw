...
# orderly::orderly_develop_start("tcd_survey_behav_men")
# setwd("src/tcd_survey_behav_men")

#' ISO3 country code
iso3 <- "TCD"

areas <- read_sf("depends/tcd_areas.geojson") %>% st_make_valid()
areas_wide <- naomi::spread_areas(areas)

surveys <- create_surveys_dhs(iso3, survey_characteristics = 24) %>%
  filter(as.numeric(SurveyYear) > 2004)
...
write_csv(survey_indicators, "tcd_survey_indicators_sexbehav.csv", na = "")
...
write_csv(hiv_indicators, "tcd_hiv_indicators_sexbehav.csv", na = "")
