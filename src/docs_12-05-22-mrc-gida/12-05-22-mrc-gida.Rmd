...
* Prevention packages are made up of a core package, available to everyone, and more intensified interventions
* To have the greatest impact, it's important to priorisite intensified interventions to those at highest risk
...
* Alleivating small-sample sizes (that caused direct estimates to display large variance)
...
* Our risk group estimates can be used to help countries implement the Global AIDS Strategy (user guide currently under development)
* It's important to reach FSW (not a new message!)
* Countries have different epidemic profiles

## Methods takeaways

* `R-INLA` and the multinomial-Poisson trick are a good choice for spatio-temporal multinomial models
  * The `group` and `replicate` features can be used to define quite complex Kronecker product effects
  * However, can run into limitations on model complexity and I might be moving to something more flexible e.g. `tmb`, `aghq` to continue development of this project

## Thanks for listening!

* This project was joint work with members of the HIV inference group, particularly Katie Risher and Jeff Eaton
  * \faHome $\,$ `hiv-inference.org`
* You can find me via
