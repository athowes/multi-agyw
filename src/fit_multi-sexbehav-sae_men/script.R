...
stopifnot(df$year_idx %>% max() == length(1999:2021)) #' 20 years
stopifnot(
...
#' Each obs_idx should have three cat_idx contained within
# stopifnot(
#   df %>%
#     group_by(obs_idx) %>%
...
#     nrow() == 0
# )
# TEMPORARY FIX: Cut out obs_idx that have less than 3 cat_idx - i think these are
# clusters that have very few observations
df <- df %>%
  group_by(obs_idx) %>%
  filter(length(unique(cat_idx)) == 3) %>%
  ungroup()
