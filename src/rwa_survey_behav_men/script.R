...
# orderly::orderly_develop_start("rwa_survey_behav_men")
# setwd("src/rwa_survey_behav_men")

#' ISO3 country code
iso3 <- "RWA"

areas <- read_sf("depends/rwa_areas.geojson") %>% st_make_valid()
areas_wide <- naomi::spread_areas(areas)

surveys <- create_surveys_dhs(iso3, survey_characteristics = 24) %>%
  filter(as.numeric(SurveyYear) > 2005)

# survey_meta <- create_survey_meta_dhs(surveys)
# there is a french report in 2005 that is messing up the output of create_survey_meta_dhs
...
final_rep <- dplyr::select(final_rep, SurveyId, report_url = PublicationURL) %>%
  dplyr::filter(report_url!="https://www.dhsprogram.com/pubs/pdf/FR180/FR180.pdf")
surveys_formeta <- dplyr::left_join(surveys, final_rep, by = "SurveyId")
surveys_formeta$dataset_url <- paste0("https://dhsprogram.com/methodology/survey/survey-display-",
                                      surveys_formeta$SurveyNum, ".cfm")
survey_meta <- dplyr::transmute(surveys_formeta, survey_id, country = CountryName,
                                survey_type = SurveyType, survey_year = SurveyYear, fieldwork_start = FieldworkStart,
                                fieldwork_end = FieldworkEnd, survey_mid_calendar_quarter,
                                female_age_min = MinAgeWomen, female_age_max = MaxAgeWomen,
                                male_age_min = MinAgeMen, male_age_max = MaxAgeMen, report_ref = NA_character_,
...
#' PHIA data
phia_survey_meta <- read_csv("depends/rwa2018phia_survey_meta.csv")
phia_survey_regions <- read_csv("depends/rwa2018phia_survey_regions.csv")
phia_survey_clusters <- read_csv("depends/rwa2018phia_survey_clusters.csv")
phia_survey_individuals <- read_csv("depends/rwa2018phia_survey_individuals.csv")
phia_survey_biomarker <- read_csv("depends/rwa2018phia_survey_biomarker.csv")
phia_survey_sexbehav <- read_csv("depends/rwa2018phia_survey_sexbehav.csv")
...
write_csv(survey_indicators, "rwa_survey_indicators_sexbehav.csv", na = "")
...
write_csv(hiv_indicators, "rwa_hiv_indicators_sexbehav.csv", na = "")
