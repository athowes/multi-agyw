...
# orderly::orderly_develop_start("tza_survey_behav_men")
# setwd("src/tza_survey_behav_men")
...
#' PHIA 2022 data
phia_2022_survey_meta <- read_csv("depends/tza2022phia_survey_meta.csv")
phia_2022_survey_regions <- read_csv("depends/tza2022phia_survey_regions.csv")
phia_2022_survey_clusters <- read_csv("depends/tza2022phia_survey_clusters.csv")
phia_2022_survey_individuals <- read_csv("depends/tza2022phia_survey_individuals.csv")
phia_2022_survey_biomarker <- read_csv("depends/tza2022phia_survey_biomarker.csv")
phia_2022_survey_sexbehav <- read_csv("depends/tza2022phia_survey_sexbehav.csv")
(phia_2022_misallocation <- check_survey_sexbehav(phia_2022_survey_sexbehav))

#' PHIA survey indicator dataset
phia_2022_survey_indicators <- calc_survey_indicators(
  phia_2022_survey_meta,
  phia_2022_survey_regions,
  phia_2022_survey_clusters,
  phia_2022_survey_individuals,
  phia_2022_survey_biomarker,
  list(phia_2022_survey_sexbehav),
  st_drop_geometry(areas),
  sex = sex,
  age_group_include = age_group_include
)
...
survey_indicators <- bind_rows(survey_indicators, phia_survey_indicators, phia_2022_survey_indicators)
...
phia_2022_survey_sexbehav_reduced <- phia_2022_survey_sexbehav %>%
  select(-sex12m, -sexcohabspouse, -sexnonregspouse, -giftsvar, -sexnonregplus, -sexnonregspouseplus)

phia_2022_hiv_indicators <- calc_survey_hiv_indicators(
  phia_2022_survey_meta,
  phia_2022_survey_regions,
  phia_2022_survey_clusters,
  phia_2022_survey_individuals,
  phia_2022_survey_biomarker,
  survey_other = list(phia_2022_survey_sexbehav_reduced),
  st_drop_geometry(areas),
  sex = sex,
  age_group_include = age_group_include,
  area_top_level = 0,
...
hiv_indicators <- bind_rows(hiv_indicators, phia_hiv_indicators, phia_2022_hiv_indicators)
...
write_csv(hiv_indicators, "tza_hiv_indicators_sexbehav.csv", na = "")
