...
# orderly::orderly_develop_start("bfa_survey_behav")
# setwd("src/bfa_survey_behav")

#' ISO3 country code
iso3 <- "BFA"

areas <- read_sf("depends/bfa_areas.geojson") %>% st_make_valid()
areas_wide <- naomi::spread_areas(areas)

surveys <- create_surveys_dhs(iso3, survey_characteristics = 24) %>%
  filter(as.numeric(SurveyYear) > 2000)
...
write_csv(survey_indicators, "bfa_survey_indicators_sexbehav.csv", na = "")
...
write_csv(hiv_indicators, "bfa_hiv_indicators_sexbehav.csv", na = "")
