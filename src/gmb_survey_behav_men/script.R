...
# orderly::orderly_develop_start("gmb_survey_behav_men")
# setwd("src/gmb_survey_behav_men")

#' ISO3 country code
iso3 <- "GMB"

areas <- read_sf("depends/gmb_areas.geojson") %>% st_make_valid()
...
# GMB2013DHS                1             Banjul
# GMB2013DHS                7        Janjanbureh
# GMB2013DHS                2           Kanifing
# GMB2019DHS                1             Banjul
# GMB2019DHS                7        Janjanbureh
# GMB2019DHS                2           Kanifing

#' Manually add the districts that intersect Banjul, Janjanbureh & Kanifing
#' survey region to survey_region_areas

gmb2013_2019dhs_region1_areas <- areas_wide %>%
  st_join(
    survey_region_boundaries %>%
      filter(survey_id == "GMB2013DHS" | survey_id=="GMB2019DHS", survey_region_id %in% c(1,2,7)),
    left = FALSE
  ) %>%
  select(all_of(names(survey_region_areas)))

survey_region_areas <- survey_region_areas %>%
  bind_rows(gmb2013_2019dhs_region1_areas)
...
write_csv(survey_indicators, "gmb_survey_indicators_sexbehav.csv", na = "")
...
write_csv(hiv_indicators, "gmb_hiv_indicators_sexbehav.csv", na = "")
