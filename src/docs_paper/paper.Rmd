...
abstract: |
  ...
  **Results:**
  ...
  Prioritisation based on behavioural risk, in combiniation with location and age based prioritisation, reduced the proportion of population that must be reached to find half of all new infections from `r "num"`\% to `r "num"`\%.
...
author_summary: |
  The risk HIV poses to adolescent girls and young women depends on the extent of HIV in the local community and their individual sexual behaviour.
  To help policy makers identify and provide services to those at highest risk, we geographically mapped HIV risk using behavioural data from household surveys.
...
These risk groups were: not sexually active, one cohabiting sexual partner, at least one non-regular sexual partner, and AGYW who are female sex workers (FSW) (Table, Fig).
...
The administrative area hierarchy and geographic boundaries we used correspond to those used in Naomi, with the exception of Cameroon and Kenya where we used an analysis level one higher.
...
```{r echo=FALSE, fig.cap="Surveys used in our analysis by year, survey type, sample size, and whether the survey included a transactional sex question. \\label{fig:available-surveys}"}
...
Model performance according to the CPO criteria is provided in the appendix (Figs, Tables) for all considered models.

## Risk group estimates

```{r continental-map, echo=FALSE, fig.cap="Choropleths of the posterior mean proportion of AGYW in each risk group in 2018. Countries in grey are included to show Cameroon (upper left) in relation to the other twelve spatially-contiguous countries (lower right). \\label{fig:continental-map}"}
knitr::include_graphics("depends/3p1-continental-map.pdf")
```

```{r within-between-country-variation, echo=FALSE, fig.cap="National (in white) and subnational (in color) posterior means of the risk group proportions. We used regions of sub-Saharan Africa defined according to the UN geocode (M49). \\ref{fig:within-between-country-variation}"}
...
In the sections to follow, all estimates refer to 2018, the most recent year included in our analysis, unless otherwise indicated.
...
The exception was Mozambique, where the majority (`r 100 - round(moz_Y015_019_nosex12m$estimate_smoothed, 2)`\%; 95\% CI `r "num"`--`r "num"`\%) were sexually active in the past year and close to a third (`r round(moz_Y015_019_sexcohab$estimate_smoothed, 2)`\%; 95\% CI `r "num"`--`r "num"`\%) were cohabiting with a partner.
...
```{r echo=FALSE, fig.cap="Percentage of new infections reached across all 13 countries, taking a variety of risk stratification approaches, against the percentage of at risk population reached. \\ref{fig:infections-reached}"}
...
Reaching half of all new infections required reaching `r "num"`\% of the population when stratifying by area and age, but only `r "num"`\% when behavioural stratification was included (Fig \ref{fig:infections-reached}).
The majority of this benefit came from reaching FSW, who were `r "num"`\% of the population but `r "num"`\% of all new infections.

Efficiency was reduced if prioritisation was done separately by country.
On average, reaching half of new infections in a country required reaching `r "num"`\% (range `r "num"`-`r "num"`\%) of the population when stratifying by area and age, reducing to `r "num"`\% (range `r "num"`-`r "num"`\%) when behaviour was included.
The importance of stratifying risk upon age, location and behaviour within a country varied, analogous to the varying contribution of each to the total variance (Sec \@ref(analysis-of-variance)).
For example, FSW in Tanzania were estimated to be `r "num"`\% of the population and close to half (`r "num"`\%) of all new infections, whereas FSW in Namibia were just `r "num"`\% of the population and `r "num"`\% of all new infections.
