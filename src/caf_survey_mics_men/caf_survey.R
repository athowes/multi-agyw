...
mics <- ind %>%
  filter(MWM17 == 1) %>% # filter out anyone who didn't complete the survey
  select(MWM1, MWM2, MWM3, MWM6Y, MWM6M, # DO I NEED URBAN AND REGION IN HERE?  Only in HH
         PSU, mnweight, stratum,MWB4,
         ethnicity, religion)  %>%
  mutate( personid = paste0(MWM1,"_",MWM2,"_",MWM3),
          gender = "male",
          survey_id = survey_id,
          cluster_id = MWM1)
