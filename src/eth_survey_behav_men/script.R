...
# orderly::orderly_develop_start("eth_survey_behav_men")
# setwd("src/eth_survey_behav_men")
...
surveys <- create_surveys_dhs(iso3, survey_characteristics = 24) %>%
  ...
  filter(as.numeric(SurveyYear) != 2005) #HAVING ISSUES NOW WITH 2005 SURVEY - COME BACK TO THIS!
...
#' ETH2016DHS: Both Gambela and Harari assigned to region = 8
#' Recode Harari to survey_region_id = 9

survey_region_boundaries[survey_region_boundaries$survey_id == "ETH2016DHS", ]

survey_region_boundaries$survey_region_id[
  survey_region_boundaries$survey_id == "ETH2016DHS" &
    survey_region_boundaries$survey_region_name == "Harari"] <- 9L
...
#' PHIA data
phia_survey_meta <- read_csv("depends/eth2017phia_survey_meta.csv")
phia_survey_regions <- read_csv("depends/eth2017phia_survey_regions.csv")
phia_survey_clusters <- read_csv("depends/eth2017phia_survey_clusters.csv")
phia_survey_individuals <- read_csv("depends/eth2017phia_survey_individuals.csv")
phia_survey_biomarker <- read_csv("depends/eth2017phia_survey_biomarker.csv")
phia_survey_sexbehav <- read_csv("depends/eth2017phia_survey_sexbehav.csv")
...
write_csv(survey_indicators, "eth_survey_indicators_sexbehav.csv", na = "")
...
write_csv(hiv_indicators, "eth_hiv_indicators_sexbehav.csv", na = "")
